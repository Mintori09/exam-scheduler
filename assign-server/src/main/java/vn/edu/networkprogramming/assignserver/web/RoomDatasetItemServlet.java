...
public class RoomDatasetItemServlet extends BaseJsonServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try {
            ...
            if (parts.length == 2 && "archive".equals(parts[1])) {
                assignmentService().archiveRoomDataset(parts[0]);
                writeJson(resp, HttpServletResponse.SC_OK, new ApiErrorResponse("SUCCESS", "Đã ẩn bộ dữ liệu phòng"));
                return;
            }
            if (parts.length == 2 && "rename".equals(parts[1])) {
                assignmentService().renameRoomDataset(parts[0], req.getParameter("name"));
                writeJson(resp, HttpServletResponse.SC_OK, new ApiErrorResponse("SUCCESS", "Đã đổi tên bộ dữ liệu phòng"));
                return;
            }
            if (parts.length == 2 && "restore".equals(parts[1])) {
                assignmentService().unarchiveRoomDataset(parts[0]);
                writeJson(resp, HttpServletResponse.SC_OK, new ApiErrorResponse("SUCCESS", "Đã hiện lại bộ dữ liệu phòng"));
                return;
            }
